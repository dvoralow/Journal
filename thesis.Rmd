...
ggplot(net_mediter_data[net_mediter_data$Season=="winter",], aes(x=SST ,y=mtc, size=3 ))+
...
ggplot(net_mediter_data[net_mediter_data$Season=="spring",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
ggplot(net_mediter_data[net_mediter_data$Season=="autumn",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
ggplot(net_mediter_data[net_mediter_data$Season=="summer",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
#red sea
ggplot(net_red_data[net_red_data$Season=="winter",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
ggplot(net_red_data[net_red_data$Season=="spring",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
ggplot(net_red_data[net_red_data$Season=="autumn",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
ggplot(net_red_data[net_red_data$Season=="summer",], aes(x=SST ,y=mtc, size=3 ))+
  geom_point(color = "green", size = 1.5)+
    scale_size(guide = 'none')+#no legend to the "size"
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+#size of text
  labs(y ="MTC")+#axis text
...
GAM_models_mtc_sum=function(data_in,y_par){
  ...
  temp = data_in %>% dplyr::select(y_par,Year, Month, Depth,Fisherman,Start_hh )
...
#3
GAMM_auto_mtc_sum=function(data_in,y_par){
  
  #create Net_number and YearMonth columns for the autocorrelation
  temp = data_in %>% dplyr::select(y_par,Year, Month, Depth,Fisherman,Start_hh, DateTime )
  temp = as.data.frame(temp)
  ...
  print("simp")
   ...
   simp_1 <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12),correlation=corARMA(form = ~Net_num|YearMonth,p=1), data = temp)
  ...
  simp_5 <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12),correlation=corARMA(form = ~Net_num|YearMonth,p=5), data = temp)
  ...
  co_1 <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=1), data = temp)
  ...
  co_5 <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=5), data = temp)
  
  print("co_5")
 
  
  #create list of model outputs
  gamm_list=list(simp, simp_1, simp_3, simp_5, co, co_1, co_3, co_5)
  
  names(gamm_list)=c("simp", "simp_1", "simp_3", "simp_5", "co", "co_1", "co_3", "co_5")
...
net_all_data = net_all_data %>% filter(Year > 1990) %>% dplyr::select(-Depth) %>%
   rename(Depth = depth_meter) %>%  dplyr:: filter(!is.na(mtc))  %>% dplyr::filter(Depth<100)%>% replace_na(list(Gambari_net = FALSE)) %>% filter(Gambari_net==FALSE) %>% mutate(Date_num = as.numeric(Date))
#---seperate by origin---
...
#get out rows without mtc and depth deepre than 100 m 
 ...
 net_red_mtc = net_red_mtc %>% filter(Year > 1990)  %>% dplyr::select(-Depth) %>% rename(Depth = depth_meter) %>%  dplyr:: filter(!is.na(mtc)) %>% dplyr::filter(Depth<100)%>% replace_na(list(Gambari_net = FALSE)) %>% filter(Gambari_net==FALSE) %>% mutate(Date_num = as.numeric(Date))%>% filter(Year>1990)
...
red_catch_gamm = GAMM_auto_mtc_sum(net_red_mtc,"mtc")
aic_red_catch = AIC_GAM_auto(red_catch_gamm)


med_catch_gamm = GAMM_auto_mtc_sum(net_medietr_mtc,"mtc")
aic_med_catch = AIC_GAM_auto(med_catch_gamm)

all_catch_gamm = GAMM_auto_mtc_sum(net_all_data,"mtc")
aic_all_catch = AIC_GAM_auto(all_catch_gamm)
...
plot(gam_all,se=TRUE,col = "blue")
  ...
  summary(mediter_gam_month)
    par(mfrow = c(2,2))
      gam.check(mediter_gam_month)
...
#(Fisherman)+ s(Start_hh, bs = "cr")+s(Date_num, bs = "gp")  + 
  ...
  summary(red_gam_month)
    par(mfrow = c(2,2))
      gam.check(red_gam_month)
...
plot( sm(red_gam_month, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "red")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 1))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2))  +ggtitle("Red Sea species")


plot( sm(gam_all, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("whole known origin species")



plot( sm(mediter_gam_month, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("mediteranian species")
...
temp = net_mediter_mtc %>% dplyr::select(Year, Month, Depth, mtc ) 
...
#----biomass model - sum of catch and by month, yaer and depth-------
```{r}
#create Sum columns - sum of the catch of any net
tempor_sum = net_all_data %>% dplyr::select(Aal:WHT) 
 catch_net_sum = rowSums(tempor_sum)
 net_all_data = net_all_data %>% dplyr::mutate(Sum = cbind(catch_net_sum)) 
...
tempor_sum = net_mediter_mtc %>% dplyr::select(Aal:WHT) 
 ...
 net_all_data = net_all_data %>% filter(Year<2018)
...
all_catch_gam = GAM_models_mtc_sum(net_all_data,"Sum")
aic_all_catch = AIC_GAM_F(all_catch_gam)
    
 par(mfrow = c(2,2))
  plot(  all_catch_gam[[1]],se=TRUE,col = "blue") 
...
aic_med_catch = AIC_GAM_auto(med_catch_gamm)
 ...
 #net cath model
 all_catch_gam = gam(Sum ~ s(Year, bs = "cr") +s(Month, bs = "cc", k=12)+s(Depth), data = net_all_data)
 
 summary(all_catch_gam)
  gam.check(all_catch_gam)
  
  par(mfrow = c(2,2)) 
  plot(  all_catch_gam,se=TRUE,col = "blue")
    title("whole known origin species catch by month and year",line = -2, outer=TRUE)
  ...
  summary(mediter_catch_gam)
  gam.check(mediter_catch_gam)
  ...
  plot(  mediter_catch_gam,se=TRUE,col = "blue")
    ...
    red_catch_gam = gam(Sum ~ s(Year, bs = "cr") +s(Month, bs = "cc", k=12)+s(Depth), data = net_red_mtc)
     ...
     summary(red_catch_gam)
     par(mfrow = c(2,2)) 
  gam.check(red_catch_gam)
...
plot( sm(all_catch_gam, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("whole known origin species")
 
plot( sm(mediter_catch_gam, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("Mediterranean species")

plot( sm(red_catch_gam, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "red")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("Red Sea species")
...
all_dataF = read.csv(file = "prosseced data/all_data_fishes_only.csv", stringsAsFactors = F)
 ...
 # set NA to FALSE and remove rows with gamberi nets, the columns "fish (FIS)", "general box (BOX)", Speciel fish boxes (SPL), start from 1991, remove depth deeper than 100 m, and stay with 1 depth column, called "Depth"
  all_dataF = all_dataF %>% replace_na(list(Gambari_net=FALSE)) %>% dplyr::filter(Gambari_net=="FALSE") %>% dplyr::select( everything(),-BOX, -FIS, -SPL, -loc_DEPTH, -Depth) %>% filter(Year>1990) %>% rename(Depth = depth_meter) %>% filter(Depth<100)%>% replace_na(list(Gambari_net = FALSE)) %>% filter(Gambari_net==FALSE)
...
plot( sm(all_catch_gam, 1) )+ l_ciPoly(fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("whole fish species")
...
print(abondance_species[i])
  ...
  #create list of model outputs
   species_gam_list =list(simp, co, simp_int, co_int)
 # paste("gam",abondance_species[i],"list",sep = "_") =list(simp, co, simp_int, co_int)
  
  names(species_gam_list)=c("simp","co","simp_int","co_int")
