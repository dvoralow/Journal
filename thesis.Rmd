...
input:
  1. data_in = the data to make gam on
  2. y_par - y axis paramater - mtc, Sum or species name, as character
  3. tw_family - TRUE if I want the gam model to by in tweedie family
...
3. AIC_GAM_auto - create table of AIC for the autocorelation model get inside
...
#get out rows without mtc and depth deepre than 100 m 
 ...
 net_red_mtc = net_red_mtc %>% filter(Year > 1990)  %>% dplyr::select(-Depth) %>% rename(Depth = depth_meter) %>%  dplyr:: filter(!is.na(mtc)) %>% dplyr::filter(Depth<100)%>% replace_na(list(Gambari_net = FALSE)) %>% filter(Gambari_net==FALSE) %>% mutate(Date_num = as.numeric(Date))%>% filter(Year>1999)
...
y = aic_all_catchF %>% arrange(AIC)
y = aic_all_catchF %>% lapply( function(x){
  arrange(x,AIC)
})
...
species_b = species_b[!species_b%in% c("Sud","Sja","SPH","Sd")]
b_size_all = b_size_all %>%dplyr::filter(!species %in% c("Sud","Sja","SPH","Sd"))
...
co_int = gam(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+ s(Start_Time_hh, bs = "cr"), data = temp, family = tw)
