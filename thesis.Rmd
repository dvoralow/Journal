...
#---mean season depth and temperature
     season_depth=dplyr:: select(all_data,Year,Season_num,loc_DEPTH)
    #remove 0 and NA
    season_depth=na.omit(season_depth)
    season_depth=season_depth[season_depth$loc_DEPTH!=0,]
    #aggregate by year by season
    
    #season_depth =season_depth %>% dplyr::mutate(Season_num = ifelse(Season=="winter", 1, ifelse(Season=="spring", 2, ifelse(Season=="summer", 3, ifelse(Season=="autumn", 4, NA))))) %>% dplyr::select(Year, Season_num, loc_DEPTH)
    ...
    season_depth=na.omit(season_depth)
    season_depth=dplyr:: select(season_depth,Year,Season_num,Depth=loc_DEPTH)
    ...
    season_temp=season_temp[season_temp$SST!=0,]
     #season_temp =season_temp %>% dplyr::mutate(Season_num = ifelse(Season=="winter", 1, ifelse(Season=="spring", 2, ifelse(Season=="summer", 3, ifelse(Season=="autumn", 4, NA))))) %>% dplyr::select(Year, Season_num, SST)
    season_temp=aggregate.data.frame(season_temp,list(season_temp$Year,season_temp$Season_num),FUN=mean,drop = F)
    season_temp=na.omit(season_temp)
    season_temp=dplyr:: select(season_temp,Year,Season_num,SST)
