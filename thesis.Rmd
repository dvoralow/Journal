...
all_dataF = read.csv(file = "prosseced data/all_data_fishes_only.csv", stringsAsFactors = F)
 ...
 # set NA to FALSE and remove rows with gamberi nets, and the columns "fish (FIS)", "general box (BOX)"
  all_dataF = all_dataF %>% replace_na(list(Gambari_net=FALSE)) %>% dplyr::filter(Gambari_net=="FALSE") %>% dplyr::select( everything(),-BOX, -FIS) %>% filter(Year>1990)
...
temp_gam2 = gam(temp[,4] ~ te(Month,Year, bs = c("cc","cr"))+s(depth_meter) ,data = temp)

temp_gam2 = getViz(temp_gam2)


print(plot(sm(temp_gam2,1)) + labs(title = paste(sp_codeF[i],scientific_codeF[i],sep = "-")), pages = 1)
    ...
    title(paste(sp_codeF[i], scientific_codeF[i],sep = " "),line = -2, outer=TRUE)
