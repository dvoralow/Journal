...
#---loop - GAM models, AIC test, and graphs for phenology and seasonality of spcies--
...
print(species_b[i])
 ...
 co <- bam(temp[,1] ~ s(Year, bs = "cr")  +  s(Month, bs = "cc", k=12)+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), data = temp )
 print("co")
  
  co_1 <- bam(temp[,1] ~ s(Year, bs = "cr")  +  s(Month, bs = "cc", k=12)+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.1, data = temp )
  
  print("co_1") 
   
  co_3 <- bam(temp[,1] ~ s(Year, bs = "cr")  +  s(Month, bs = "cc", k=12)+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.3, data = temp )
  
  print("co_3")
  
  co_5 <- bam(temp[,1] ~ s(Year, bs = "cr")  +  s(Month, bs = "cc", k=12)+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.5, data = temp )
  
  print("co_5")
 
  co_int <- bam(temp[,1] ~ te(Year, bs = "cr") + te(Month, bs = "cc", k=12)   + ti(Month,Year, bs = c("cc","cr"))  + (Fishermen)+ s(Start_Time_hh, bs = "cr"), data = temp )
 print("co_int")
  
  co_1_int <- bam(temp[,1] ~ te(Year, bs = "cr") + te(Month, bs = "cc", k=12)  + ti(Month,Year, bs = c("cc","cr"))  + (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.1, data = temp )
  
  print("co_1_int") 
   
  co_3_int <- bam(temp[,1] ~ te(Year, bs = "cr") + te(Month, bs = "cc", k=12) + ti(Month,Year, bs = c("cc","cr"))  + (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.3, data = temp )
  
  print("co_3_int")
  
  co_5_int <- bam(temp[,1] ~ te(Year, bs = "cr") + te(Month, bs = "cc", k=12)  + ti(Month,Year, bs = c("cc","cr"))  + (Fishermen)+ s(Start_Time_hh, bs = "cr"), rho = 0.5, data = temp )
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = paste( species_b_data[i,3],catch_b_years$Year_S[i],"-",catch_b_years$Year_E[i],sep = " "),
         x = NULL,
         y = "Relative catch") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)+
  theme_update(text = element_text(size=20))
