...
aic_red_mtc = AIC_GAM_F(all_mtc_gam)
...
pdata <- with(temp, expand.grid(Year = seq(min(Year), max(Year), length.out = 100),
                                  Month = seq(1, 12, length.out = 100), Depth =30,S))

## Add on the fitted values and SEs *on the log [link] scale*
pdata <- cbind(pdata, as.data.frame(predict(temp_gam$gam, newdata = pdata, type = "link", se.fit = TRUE)))
...
net_mediter_mtc = net_mediter_mtc %>% filter(Year<2018)
 net_red_mtc = net_red_mtc %>% filter(Year<2018)
 #%>% filter(Year>1999)
 

 
 
  #------call the function on Sum  gams
red_catch_gam = GAM_models_mtc_sum(net_red_mtc,"Sum")
aic_red_catch = AIC_GAM_F(red_catch_gam)

 par(mfrow = c(2,2))
  plot(  red_catch_gam[[1]],se=TRUE,col = "blue") 
