...
GAM_models_mtc_sum=function(data_in,y_par){
  ...
  co <- gam(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"), data = temp)
  ...
  co_int = gam(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"), data = temp)
...
GAMM_auto_mtc_sum=function(net_data,y_par){
  ...
  summary(simp_3$gam)
   ...
   co <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"), data = temp)
  
   summary(co$gam)
   
   
  co_3 <- gamm(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = temp)
  
  summary(co_3$gam)
  
  co_int = gamm(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = temp)
...
write.csv(aic_mtc,"C:/Users/dvora/Google Drive/second degree/thesis/aic_mtc_bam.CSV")


#---call to autocorrelation gamm model and AIC - the function does'nt work, but work wiht handy running
red_mtc_gamm = GAMM_auto_mtc_sum(net_red_mtc,"mtc")
aic_red_mtc = AIC_GAM_auto(red_mtc_gamm)


med_mtc_gamm = GAMM_auto_mtc_sum(net_medietr_mtc,"mtc")
aic_med_mtc = AIC_GAM_auto(med_mtc_gamm)

all_mtc_gamm = GAMM_auto_mtc_sum(net_all_data,"mtc")
aic_all_mtc = AIC_GAM_auto(all_mtc_gamm)
    
row.names(aic_red_mtc) = c("red","2","3","4","5","6")
row.names(aic_all_mtc) = c("all","2","3","4","5","6")
row.names(aic_med_mtc) = c("med","2","3","4","5","6")

aic_mtc = rbind(aic_all_mtc, aic_med_mtc, aic_red_mtc)
write.csv(aic_mtc,"prosseced data/aic_mtc_gamm.CSV")
...
```{r}
 ...
 #all data gam model
gam_all = gamm(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+ s(Start_hh, bs = "cr")+s(Depth)+ (Fisherman),correlation=corARMA(form = ~Net_num|YearMonth,p=1), data = net_all_data)

gam_all = gamm(mtc ~ s(Year, bs = "cr")+ (Fisherman)+ s(Start_hh, bs = "cr") + s(Depth)   + 
                             s(Month, bs = "cc", k=12),correlation=corARMA(form = ~Net_num|YearMonth,p=1),data =net_all_data)
#
#gam_all = gam(mtc ~ s(Year, bs = "cr")+ (Fisherman)+ s(Start_hh, bs = "cr") + s(Depth)   + 
                           #  s(Month, bs = "cc", k=12),data =net_all_data)
#+ (Fisherman)+ s(Start_hh, bs = "cr")+s(Date_num, bs = "gp")
...
mediter_gam_month = gamm(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_mediter_mtc)

 # mediter_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + (Fisherman)+ s(Start_hh, bs = "cr")   +s(Month, bs = "cc", k=12),data =net_mediter_mtc)
...
par(mfrow = c(2,2)) 
plot(mediter_gam_month$gam,se=TRUE,col = "blue")
   # net_red_mtc = net_red_mtc %>% filter(Year>1999)

red_gam_month = gamm(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_red_mtc)
  
red_gam_month = bam(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),rho = 0.5, data = net_red_mtc)

 # red_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + (Fisherman)+ s(Start_hh, bs = "cr")   +s(Month, bs = "cc", k=12) ,data = net_red_mtc,method = "REML")
...
#(Fisherman)+ s(Start_hh, bs = "cr")+s(Date_num, bs = "gp")  + 
  ...
  summary(red_gam_month$gam)
    par(mfrow = c(2,2))
      ...
      par(mfrow = c(2,2)) 
plot(red_gam_month$gam,se=TRUE,col = "blue")
  
  #---------------nice graphs for thesis-------------
  gam_all = getViz(gam_all$gam)
mediter_gam_month <- getViz(mediter_gam_month$gam)
red_gam_month = getViz(red_gam_month$gam)
red_gam_month = getViz(red_gam_month)
#red_gam_month = getViz(red_gam_month$gam)

plot( sm(red_gam_month, 2) )+ l_ciPoly(level = 0.95,fill = "grey")+l_fitLine(colour = "red")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 1))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2))  +ggtitle("MTC model - Red Sea species")


plot( sm(gam_all, 1) )+ l_ciPoly(level = 0.95,fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("whole known origin species")



plot( sm(mediter_gam_month, 2) )+ l_ciPoly(level = 0.95,fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Temperature affinity")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("MTC model - mediteranian species")
...
gamm_red = gamm(mtc ~ s(Year, bs = "cr")+ (Fisherman)+ s(Start_hh, bs = "cr") + s(Depth)   + 
...
temp = net_all_data %>% dplyr::select(Year, Month, Depth, mtc ) 
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    ...
    labs(title = "Known origin species mtc interaction model",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Temperature affinity") +
...
temp = net_mediter_mtc %>% dplyr::select(Year, Month, Depth, mtc, Net_num, YearMonth ) 
  
temp = as.data.frame(temp)
  
   temp_gam = gam(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth) ,data = temp)
   
    temp_bam = bam(mtc ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth),rho = 0.7 ,data = temp)
    #
...
ilink <- family(temp_gam)$linkinv   # this is the inverse of the link function, as an R function
ilink <- family(temp_bam)$linkinv   # this is the inverse of the link function, as an R function
...
## Add on the fitted values and SEs *on the log [link] scale*
pdata <- cbind(pdata, as.data.frame(predict(temp_gam, newdata = pdata, type = "link", se.fit = TRUE)))
pdata <- cbind(pdata, as.data.frame(predict(temp_bam, newdata = pdata, type = "link", se.fit = TRUE)))
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Mediterranean species mtc interaction model",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Temperature affinity") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)
...
#red sea interaction
temp = net_red_mtc %>% dplyr::select(Year, Month, Depth, mtc )
temp = as.data.frame(temp)
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Red Sea species mtc interaction model",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Temperature affinity") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)
...
aic_catch = rbind(aic_red_catch,aic_med_catch)
   ...
   #------call the autocorrelation function on Sum  gamms - whith autocorrelation
  #- the function does'nt work, but work wiht handy running
red_catch_gamm = GAMM_auto_mtc_sum(net_red_mtc,"Sum")
aic_red_catch = AIC_GAM_auto(red_catch_gamm)


med_catch_gamm = GAMM_auto_mtc_sum(net_medietr_mtc,"Sum")
aic_med_catch = AIC_GAM_auto(med_catch_gamm)

 row.names(aic_red_catch) = c("red","2","3","4","5","6")
row.names(aic_med_catch) = c("med","2","3","4","5","6") 
...
mediter_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_mediter_mtc)

#without interaction
mediter_catch_gam = gamm(Sum ~ s(Month, bs = "cc", k=12) + s(Year, bs = "cr")  +s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_mediter_mtc)
    
  #mediter_catch_gam = gam(Sum ~ s(Year, bs = "cr") +s(Month, bs = "cc", k=12)+s(Depth), data = net_mediter_mtc )
  ...
  plot(  mediter_catch_gam$gam,se=TRUE,col = "blue")
   ...
   red_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_red_mtc)
   
   #without interaction
    red_catch_gam = gamm(Sum ~ s(Month, bs = "cc", k=12) + s(Year, bs = "cr")  +s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_red_mtc)
   
    #red_catch_gam = gam(Sum ~ s(Year, bs = "cr") +s(Month, bs = "cc", k=12)+s(Depth), data = net_red_mtc)
...
#nice plots  
mediter_catch_gam <- getViz(mediter_catch_gam$gam)
red_catch_gam = getViz(red_catch_gam$gam)

 
plot( sm(mediter_catch_gam, 2) )+ l_ciPoly(level = 0.95,fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("Biomass - Mediterranean species")

plot( sm(red_catch_gam, 2) )+ l_ciPoly(level = 0.95,fill = "grey")+l_fitLine(colour = "red")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="Relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("Biomass - Red Sea species")
...
# catch interaction model and graph
   temp = net_all_data %>% dplyr::select(Year, Month, Depth, Sum ) 
...
pdata <- with(temp, expand.grid(Year = seq(min(Year), max(Year), length.out = 100),
                                  Month = seq(1, 12, length.out = 100), Depth =30 ))
#seq(min(Depth), max(Depth), length.out = 100)
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Whole known origin catch",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
...
#mediter interaction model
 temp = net_mediter_mtc %>% dplyr::select(Year, Month, Depth, Sum ) 
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Mediter species catch",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Estimated total catch (kg))") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)
...
#red sea interaction
temp = net_red_mtc %>% dplyr::select(Year, Month, Depth, Sum ) 
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Red Sea species catch",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Estimated total catch (kg))") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)
...
all_dataF = read.csv(file = "prosseced data/all_data_fishes_only.csv", stringsAsFactors = F)
 ...
 # set NA to FALSE and remove rows with gamberi nets, the columns "fish (FIS)", "general box (BOX)", Speciel fish boxes (SPL), start from 1991, remove depth deeper than 100 m, and stay with 1 depth column, called "Depth"
  all_dataF = all_dataF %>% replace_na(list(Gambari_net=FALSE)) %>% dplyr::filter(Gambari_net=="FALSE") %>% dplyr::select( everything(),-BOX, -FIS, -SPL, -loc_DEPTH, -Depth) %>% filter(Year>1990) %>% rename(Depth = depth_meter) %>% filter(Depth<100)%>% replace_na(list(Gambari_net = FALSE)) %>% filter(Gambari_net==FALSE) %>% arrange(DateTime) 
...
#--gam model for the whole data cathc - regular gam graph, and phenology interaction graph
...
aic_all_catchF = AIC_GAM_F(all_catchF_gam)
 ...
 par(mfrow = c(2,2))
  plot(  all_catchF_gam[[1]],se=TRUE,col = "blue")
  
 #call to auotcorrelation gamm and AIC functions
  #- the function does'nt work, but work wiht handy running
  
all_catchF_gamm = GAMM_auto_mtc_sum(all_dataF,"Sum")
aic_all_catchF = AIC_GAM_auto(all_catchF_gamm)

row.names(aic_all_catchF) = c("all","2","3","4","5","6")

aic_catch = rbind(aic_catch,aic_all_catchF)

write.csv(aic_catch,"prosseced data/aic_catch_gamm.CSV")

all_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+ s(Start_hh, bs = "cr")+s(Depth)+ (Fisherman),correlation=corARMA(form = ~Net_num|YearMonth,p=1), data = all_dataF)
 ...
 summary(all_catch_gam$gam)
  ...
  plot(  all_catch_gam$gam,se=TRUE,col = "blue")
    ...
    all_catch_gam = getViz(all_catch_gam$gam)

plot( sm(all_catch_gam, 2) )+ l_ciPoly(level = 0.95 ,fill = "grey")+l_fitLine(colour = "blue")  + theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1),axis.title.x = element_text(vjust = -2),axis.title.y = element_text(vjust = 3))+labs(y ="relative catch")+scale_x_continuous (breaks=seq(1991,2018, by = 2)) +ggtitle("whole fish species")


# interaction model and graph
   temp = all_dataF %>% dplyr::select(Year, Month, Depth, Sum ) 
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    geom_line() +
    scale_color_viridis(option = "plasma", end = 0.95) +
    labs(title = "Whole fish data catch",
         subtitle = "Marked change in within-year distribution",
         x = NULL,
         y = "Estimated total catch (kg))") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)
...
#---GAM models and AIC for phenology of spcies--
...
for (i in 1:length(abondance_species))
{
   ...
   temp = all_dataF %>% dplyr::select(abondance_species[i], Year, Month, Depth, Fisherman,Start_hh,Net_num,YearMonth   ) %>% filter(Year >= catch_sum$Year_S[i] )  %>% filter(Year <= catch_sum$Year_E[i] )
...
print(abondance_species[i])
 ...
 print(simp_3$gam)
  ...
  co_int = gamm(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"),correlation=corARMA(form = ~Net_num|YearMonth,p=2), data = temp)

    
  
 #create list of model outputs
  species_gamm_list=list(simp, simp_3, simp_int, co, co_3, co_int)
  
  names(species_gamm_list)=c("simp", "simp_3", "simp_int", "co", "co_3", "co_int")
  
  
  aic_species = AIC_GAM_auto(species_gamm_list)
...
simp <- gam(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12), data = temp, family = tw)
  ...
  co <- gam(temp[,1] ~ s(Year, bs = "cr") + s(Depth) +  s(Month, bs = "cc", k=12)+ (Fisherman)+ s(Start_hh, bs = "cr"), data = temp, family = tw)
  ...
  co_int = gam(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cr"), data = temp, family = tw)

  print(co_int)


#--loop - create interaction graphs for the whole species
```{r}
...
print(species_b[i])
       ...
       simp <- gam(temp[,1] ~ s(Year, bs = "cr") +  s(Month, bs = "cc", k=12), data = temp, family = tw)
  
  print(simp)

   co <- gam(temp[,1] ~ s(Year, bs = "cr")  +  s(Month, bs = "cc", k=12)+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), data = temp, family = tw)
  
  print(co)
  
  simp_int = gam(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr")), data = temp, family = tw)
  
  print(simp_int)
  
  co_int = gam(temp[,1] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+ (Fishermen)+ s(Start_Time_hh, bs = "cr"), data = temp, family = tw)

  print(co_int)
  
  #create list of model outputs
   bsize_gam_list =list(simp, co, simp_int, co_int)
 # paste("gam",abondance_species[i],"list",sep = "_") =list(simp, co, simp_int, co_int)
  
  names(bsize_gam_list)=c("simp","co","simp_int","co_int")
  
  aic_bsize = AIC_GAM_F(bsize_gam_list)

 aic_bsize_list[[i]] =aic_bsize
 
}#end for

#temp_species_b = species_b[!species_b %in% "Umo"]
names(aic_bsize_list) = temp_species_b

aic_bsize_list = aic_bsize_list %>% lapply(function(x){
  arrange(x,AIC) %>% 
  mutate(delta = AIC - first(AIC))
})
