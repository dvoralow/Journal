...
theme_set(theme_bw())
 ...
 EPI_gam = gam(EPI_d[,4] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "tp", k=27)  + ti(Month,Year, bs = c("cc","tp"), k=c(12,27))+s(Depth) ,data = EPI_d, family = tw)
 

  summary(EPI_gam)
  gam.check(EPI_gam)
  
#  EPI_bam = bam(EPI_d[,4] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "tp", k=27)  + ti(Month,Year, bs = c("cc","tp"), k=c(12,27))+s(Depth) ,data = EPI_d, family = tw,select = TRUE)
 # summary(EPI_bam)
  
  
  plot(EPI_gam, scheme = 2, n = 200, tran = family(EPI_gam)$linkinv)
  
  ## Look at the fitted model
ilink <- family(EPI_gam)$linkinv   # this is the inverse of the link function, as an R function

## Generate new data to predict at; to get smooth plots use ~ 100 evenly spaced values for each covariate
pdata <- with(EPI_d, expand.grid(Year = seq(min(Year), max(Year), length.out = 100),
...
pdata <- cbind(pdata, as.data.frame(predict(EPI_gam, newdata = pdata, type = "link", se.fit = TRUE)))
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year, group = factor(Year))) +
    ...
    labs(title = "Modelled change in monthly catch Lokus, 1991–2018",
