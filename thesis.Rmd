...
#get out rows without mtc and depth deepre than 100 m or shallower than 20 m
  net_mediter_mtc = net_mediter_mtc %>%  dplyr:: filter(!is.na(mtc))
  #%>% filter(depth_meter>0) %>% filter(depth_meter<100)
  net_red_mtc = net_red_mtc %>%  dplyr:: filter(!is.na(mtc))%>% filter((depth_meter<200) || (depth_meter = is.na(net_red_mtc$depth_meter)))
  #%>% filter(depth_meter>0) 
  x = net_red_mtc %>% dplyr:: filter(!is.na(depth_meter))
  x = net_red_mtc %>% dplyr:: filter(is.na(depth_meter))
 ...
 mediter_gam_Tint = gam(mtc ~ te(Month,Year, bs = c("cc","cr")) ,data = net_mediter_mtc)
 
  red_gam_Tint = gam(mtc ~ te(Month,Year, bs = c("cc","cr"),k=12),data = net_red_mtc)
 
  mediter_gam_Tint = getViz(mediter_gam_Tint)
 red_gam_Tint = getViz(red_gam_Tint)
 
plot(sm(mediter_gam_Tint, 1)) + l_fitRaster(pTrans = zto1(0.05, 2, 0.1), noiseup = TRUE) +
  l_rug() + l_fitContour()

plot(sm(red_gam_Tint, 1)) + l_fitRaster(pTrans = zto1(0.05, 2, 0.1), noiseup = TRUE) +
  l_rug() + l_fitContour()
...
mediter_gam_Dint = gam(mtc ~ te(depth_meter,Year, bs = c("cr","cr"))+s(depth_meter) ,data = net_mediter_mtc)
 
  red_gam_Dint = gam(mtc ~ te(depth_meter,Year, bs = c("cr","cr"),k=12),data = net_red_mtc)
 
  mediter_gam_ = getViz(mediter_gam_Dint)
 red_gam_ = getViz(red_gam_Dint)
 
plot(sm(mediter_gam_, 1)) + l_fitRaster(pTrans = zto1(0.05, 2, 0.1), noiseup = TRUE) +
  l_rug() + l_fitContour()

plot(sm(red_gam_, 1)) + l_fitRaster(pTrans = zto1(0.05, 2, 0.1), noiseup = TRUE) +
  l_rug() + l_fitContour()
