...
```{r}
#Scomberomorus commerson [Sco] graph
  
  
  Sco_data=dplyr:: select(week_data,Year,Week, Week_ID, Sco)
  Sco_data=week_data[week_data$Year %in% c(1998:2012) ,]
  Sco_data=Sco_data[Sco_data$Week %in% c(20:53), ]
  
  Sco_data=Sco_data[Sco_data$Year != 2011,]
  Sco_data=Sco_data[Sco_data$Year != 1999,]
  Sco_data=Sco_data[Sco_data$Year != 2007,]
  

  ggplot(Sco_data, aes(x=Week, y=Sco_data$Sco,color=Year)) +
    geom_line(size=1.5)+
    scale_color_jcolors_contin(palette="pal4") +
    facet_grid(Year~.)+
    ylab("Mean Catches Per Net")+scale_x_continuous(breaks = seq(1,53,by=4)) 
    
    
    
    #Upeneus pori UPO graph
    Upo_data=dplyr::select(week_data,Year,Week, Week_ID, Upo)
    Upo_data=week_data[week_data$Year %in% c(2005:2013) ,]
    Upo_data=Upo_data[Upo_data$Week %in% c(30:53), ]
    
    Upo_data=Upo_data[Upo_data$Year != 2012,]
    Upo_data=Upo_data[Upo_data$Year != 2013,]
    
    
    
    ggplot(Upo_data, aes(x=Week, y=Upo_data$Upo,color=Year)) +
      geom_line(size=1.5)+
      scale_color_jcolors_contin(palette="pal4") +
      facet_grid(Year~.)+ggtitle("Mean Catch of Upeneus Pori")
    ylab("Mean Catches Per Net")+scale_x_continuous(breaks = seq(1,53,by=1)) 
    
    #Truacura sp
    TRA_data=dplyr::select(week_data,Year,Week, Week_ID, TRA)
    TRA_data=week_data[week_data$Year %in% c(2003:2012) ,]
    TRA_data=TRA_data[TRA_data$Week %in% c(29:53), ]
    
    TRA_data=TRA_data[TRA_data$Year != 2011,]
    TRA_data=TRA_data[TRA_data$Year != 2012,]
    TRA_data=TRA_data[TRA_data$Year != 2013,]
    TRA_data=TRA_data[TRA_data$Year != 2007,]
    
    
    ggplot(TRA_data, aes(x=Week, y=TRA_data$TRA,color=Year)) +
      geom_line(size=1.5)+
      scale_color_jcolors_contin(palette="pal4") +
      facet_grid(Year~.)+ggtitle("Mean Catch of Truchurus sp.")+
    ylab("Mean Catches Per Net")+scale_x_continuous(breaks = seq(1,53,by=1)) 
    
    
    #Seriola Dumerili
    Sd_data=dplyr::select(week_data,Year,Week, Week_ID, Sd)
    Sd_data=week_data[week_data$Year %in% c(1998:2012) ,]
    Sd_data=Sd_data[Sd_data$Week %in% c(15:53), ]
    
    Sd_data=Sd_data[Sd_data$Year != 1999,]
    Sd_data=Sd_data[Sd_data$Year != 2007,]
    Sd_data=Sd_data[Sd_data$Year != 2013,]
    Sd_data=Sd_data[Sd_data$Year != 2007,]
    
    
    ggplot(Sd_data, aes(x=Week, y=Sd_data$Sd,color=Year)) +
      geom_line(size=1.5)+
      scale_color_jcolors_contin(palette="pal4") +
      facet_grid(Year~.)+ggtitle("Mean Catch of Seriola Dumerili")+
      ylab("Mean Catches Per Net")+scale_x_continuous(breaks = seq(1,53,by=1))
...
for (i in species_b)
{
  tepm_b_species = b_size_all %>% dplyr::select(Year.XXXX.,Month.XX.,species,kg_b) %>% filter(species == "i")
  plot(tepm_b_species)
