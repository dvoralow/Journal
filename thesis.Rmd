...
all_dataF = read.csv(file = "prosseced data/all_data_fishes_only.csv", stringsAsFactors = F)
 ...
 # set NA to FALSE and remove rows with gamberi nets, the columns "fish (FIS)", "general box (BOX)", Speciel fish boxes (SPL), start from 1991, remove depth deeper than 100 m, and stay with 1 depth column, called "Depth"
  all_dataF = all_dataF %>% replace_na(list(Gambari_net=FALSE)) %>% dplyr::filter(Gambari_net=="FALSE") %>% dplyr::select( everything(),-BOX, -FIS, -SPL, -loc_DEPTH, -Depth) %>% filter(Year>1990) %>% filter(depth_meter<100) %>% rename(Depth = depth_meter)
...
abondance_species = catch_sum$Spceis
...
EPI_d = all_dataF %>% dplyr::select(Year, Month, Depth, EPI ) %>% filter(Year >= 1991 )  %>% filter(Year <= 2018 )
  
EPI_d = as.data.frame(EPI_d)

Umo_d = all_dataF %>% dplyr::select(Year, Month, Depth, Umo ) %>% filter(Year >= 2000 )  %>% filter(Year <= 2018 )
  
Umo_d = as.data.frame(Umo_d)
  
   EPI_gam = gam(EPI_d[,4] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "tp", k=27)  + ti(Month,Year, bs = c("cc","tp"), k=c(12,27))+s(Depth) ,data = EPI_d, family = tw,select = TRUE)
 ...
 print(plot(EPI_gam) + labs(title = "Lokus", pages = 1))
...
theme_set(theme_bw())
 ...
 Umo_gam = gam(Umo_d[,4] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "tp", k=17)  + ti(Month,Year, bs = c("cc","tp"), k=c(12,17))+s(Depth) ,data = Umo_d, family = tw)
...
ilink <- family(Umo_gam)$linkinv   # this is the inverse of the link function, as an R function

## Generate new data to predict at; to get smooth plots use ~ 100 evenly spaced values for each covariate


pdata <- with(Umo_d, expand.grid(Year = seq(min(Year), max(Year), length.out = 100),
                                  Month = seq(1, 12, length.out = 100), Depth = seq(min(Depth), max(Depth), length.out = 100)))

#pdata = Umo_d %>% dplyr::select(Year,Month, Depth)
#

## Add on the fitted values and SEs *on the log [link] scale*
pdata <- cbind(pdata, as.data.frame(predict(Umo_gam, newdata = pdata, type = "link", se.fit = TRUE)))
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    ...
    labs(title = "Modelled change in monthly catch 	Upeneus pori, 2001–2018",
    ...
    p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = factor(Year),group = factor(Year))) +
...
species_b = c("Sd", "MUL", "Per", "Nra", "Sud", "Sja", "SPH", "Ccr", "Umo")
