...
ggplot(mediter_annual_mtc, aes(x=Year,y=mtc,colour=Depth, size=3))+
  geom_point()+geom_smooth(method=lm,se=FALSE,size = 1)+   # Add linear regression line, Don't add shaded confidence region
  scale_size(guide = 'none')+
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+
  labs(y ="MTC")+
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+
  geom_line(aes(y = SST_anom+20.5, colour = SST_anom))+
  scale_y_continuous(sec.axis = sec_axis(~.-20.5, name = "SST anomaly"))+
  ggtitle("Annual Mediterranian species MTC by year")

x = year_temp_depth$SST_anom
x=x+20.5
```
