...
all_dataF = read.csv(file = "prosseced data/all_data_fishes_only.csv", stringsAsFactors = F)
 ...
 catch_ex =  all_dataF %>% dplyr::group_by(Year) %>% dplyr::summarise_at(vars(Aal:WHT), list(N = ~ n()), drop = TRUE)
 x = all_dataF
 x[x==0]=NA
 catch_ex =  x %>% dplyr::group_by(Year) %>%summarise(count = sum(!is.na(Aal:WHT)))
...
for (i in 1:length(sp_codeF))
{
   ...
   temp = all_dataF %>% dplyr::select(Year, Month, Depth, sp_codeF[i])
  

  temp_gam = gam(temp[,4] ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth) ,data = temp)
