...
ggplot(season_red_mtc[season_red_mtc$Season=="autumn",], aes(x=Year ,y=mtc,colour=Depth, size=3))+
 ...
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+ #x axis years apeared
  scale_colour_gradientn(colours=topo.colors(7))+#color of the graph
  geom_line(aes(y = SST_anom+24.5, colour = SST_anom,size = 1))+#pot SST in the MTC scale
  scale_y_continuous(breaks=seq(23,27.5, by =0.5), limits = c(23,27.5),sec.axis = sec_axis(~.-24.5, name = "SST Anomaly"))+#create second axes to SST
  ggtitle("Autumn Red Sea Species MTC By Year, Compare To SST Anomaly")+
  stat_smooth(aes(y = mtc), #smooth to mtc, with no shadow
              method = "lm", formula = formula, se = FALSE, size = 1.5, color = "green",linetype = "twodash") +
  stat_smooth(aes(y =  SST_anom+24.5), #smooth to SST
              method = "lm", formula = formula, se = FALSE, size = 1, color = "blue",linetype = "twodash")
