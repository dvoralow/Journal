...
#start from 1991
annual_mtc=annual_mtc[annual_mtc$Year %in% c(1987:2013),]

plot(annual_mtc,main="annual mtc")

ggplot(annual_mtc, aes(x=Temp,y=mtc,colour=Year, size=5))+
  geom_point()+scale_color_brewer(palette="
                                  Reds")+
  ggtitle("Annual Catch MTC by mean net temperature")

ggplot(annual_mtc, aes(x=Year,y=mtc,colour=Temp, size=3))+
  geom_point()+
...
#start in 1991
mediter_annual_mtc=mediter_annual_mtc[mediter_annual_mtc$Year %in% c(1991:2013),]
red_annual_mtc=red_annual_mtc[red_annual_mtc$Year %in% c(1991:2013),]
 ...
 library(lubridate)
  ...
  all_data_unfilter$Date<-paste(all_data_unfilter$Year,all_data_unfilter$Month,all_data_unfilter$Day,sep = "-")
   ...
   week_data=week_data %>% group_by(Week_ID) %>% summarise_all(funs(sum))
    ...
    #begen in 1987
    week_data=week_data[week_data$Year %in% c(1987:2013),] 
