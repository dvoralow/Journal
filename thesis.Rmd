...
net_all_data = net_all_data %>% arrange(DateTime) %>% mutate(Net_num = c(1:nrow(net_all_data)))


gamm_all = gamm(mtc ~ s(Year, bs = "cr")+ (Fisherman)+ s(Start_hh, bs = "cr") + s(Depth)   + 
                             s(Month, bs = "cc", k=12),correlation=corAR1(form = ~Net_num),data =net_all_data)
