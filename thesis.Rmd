 ...
 #put the net temperautre into the all_data
  ...
  all_data$Date<-paste(all_data$Year,all_data$Month,all_data$Day,sep = "-")
    ...
    all_data$Date<-ymd(all_data$Date)
      ...
      season_data ["Season" = c("winter","spring", "summer", "autumn"),"Month" = c(1:12)]
...
#  plot(net_all_data$Year,net_all_data$mtc)
 ...
 #--- net mtc by origin---
  #mediter mtc
  #drop the useless data for the mtc function
    species_code=mediter_code
      net_mediter_data=dplyr:: select(mediter_all_data,X,species_code)
  ...
  #calling mtc function
    ...
    write.csv(net_all_data,"prosseced data/mediter_net_mtc.CSV")
    
    #red mtc
  #drop the useless data for the mtc function
    species_code=red_code
      net_red_data=dplyr:: select(red_all_data,X,species_code)

  #calling mtc function
    net_red_mtc=mtc_index_calc(net_red_data,red_mean_tcode)
      net_red_data=cbind(red_all_data,"mtc"=net_red_mtc$mtc)
  
    write.csv(net_all_data,"prosseced data/red_net_mtc.CSV")
...
#get out rows without mtc
  ...
  #gam model
mediter_gam_year = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH, bs = "ad") ,data = net_mediter_data)
red_gam_year = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH, bs = "ad") ,data = net_red_data)
