...
#seaon new function   
 season_catch=function(the_big_data,code)
   {
#take just the relevant column 
  filter_data=select(the_big_data,Year,Season,code)
    #sum the cathc by  by year
    filter_data=aggregate.data.frame(filter_data,list(filter_data$Year,filter_data$Season),FUN = sum,drop=F)
    #rename the new column and drop sum of years and Season   
    filter_data=select(filter_data,Year=Group.1,Season=Group.2,code ) 
   return(filter_data)
  }
...
#---mean season depth and temperature
    ...
    winter_mean_sst = mean(winter_temp$SST)
    winter_median_sst = median(winter_temp$SST)
    winter_temp = winter_temp %>% mutate("SST_anom" = SST-winter_mean_sst)
    ...
    spring_mean_sst = mean(spring_temp$SST)
    spring_median_sst = median(spring_temp$SST)
    spring_temp = spring_temp %>% mutate("SST_anom" = SST-spring_mean_sst)
    ...
    summer_mean_sst = mean(summer_temp$SST)
    summer_median_sst = median(summer_temp$SST)
    summer_temp = summer_temp %>% mutate("SST_anom" = SST-summer_mean_sst)
    ...
    autumn_mean_sst = mean(autumn_temp$SST)
    autumn_median_sst = median(autumn_temp$SST)
    autumn_temp = autumn_temp %>% mutate("SST_anom" = SST-autumn_mean_sst)
...
ggplot(mediter_annual_mtc, aes(x=Year ,y=mtc,colour=Depth, size=3))+
 ...
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+ #x axis years apeared
  ...
  stat_smooth(aes(y = mtc), #smooth to mtc, with no shadow
              method = "lm", formula = formula, se = FALSE, size = 1.5, color = "green",linetype = "twodash") +
  stat_smooth(aes(y =  SST_anom+20.5), #smooth to SST
              method = "lm", formula = formula, se = FALSE, size = 1, color = "blue",linetype = "twodash")+
...
ggplot(red_annual_mtc, aes(x=Year ,y=mtc,colour=Depth, size=3))+
 ...
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+ #x axis years apeared
  scale_colour_gradientn(colours=topo.colors(7))+#color of the graph
  geom_line(aes(y = SST_anom+26.5, colour = SST_anom,size = 1))+#pot SST in the MTC scale
  scale_y_continuous(breaks=seq(22.5,29, by =0.5), limits = c(22.5,29),sec.axis = sec_axis(~.-26.5, name = "SST Anomaly"))+#create second axes to SST
  ggtitle("Annual Red Sea Species MTC By Year, Compare To SST Anomaly")+
  stat_smooth(aes(y = mtc), #smooth to mtc, with no shadow
              method = "lm", formula = formula, se = FALSE, size = 1.5, color = "green",linetype = "twodash") +
  stat_smooth(aes(y =  SST_anom+26.5), #smooth to SST
              method = "lm", formula = formula, se = FALSE, size = 1, color = "blue",linetype = "twodash")+
...
ggplot(season_red_mtc[season_red_mtc$Season=="spring",], aes(x=Year ,y=mtc,colour=Depth, size=3))+
 ...
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+ #x axis years apeared
  ...
  scale_y_continuous(breaks=seq(22.5,28, by =0.5), limits = c(22.5,28),sec.axis = sec_axis(~.-25.5, name = "SST Anomaly"))+#create second axes to SST
