...
```{r}
 #drop the useless data for the mtc function
  ...
  net_all_data=dplyr:: select(all_data,X.3,species_code)
...
#cerate species names vector
 ...
 #remove gamberi nets
 all_data_unfilter=all_data_unfilter[all_data_unfilter$Gambari_net==FALSE,]
```
...
```{r}
 library(lubridate)
  ...
  all_data_unfilter$Date<-paste(all_data_unfilter$Year,all_data_unfilter$Month,all_data_unfilter$Day,sep = "-")
    ...
    #sum fo weekly catch of each speceis: aggergate by ID
   week_data=week_data %>% group_by(Week_ID) %>% summarise_all(funs(sum))
    ...
    #create data frame for any quart of the years
    week_data_a=  week_data[week_data$Year %in% c(1987:1993),]
    week_data_b=  week_data[week_data$Year %in% c(1994:2000),]
    week_data_c=  week_data[week_data$Year %in% c(2001:2006),]
    week_data_d=  week_data[week_data$Year %in% c(2007:2013),]
