...
```{r eval=FALSE, include=FALSE}
 #put the net temperautre into the all_data
  ...
  #cerate date column
  all_data$Date<-paste(all_data$Year,all_data$Month,all_data$Day,all_data$Start_hh,sep = "-")
    #order the column by year month day
    all_data$Date<-ymd_h(all_data$Date)
 ...
 all_data = all_data %>% dplyr::select(X:Day,Date:Season,everything())
...
```{r}
   ...
   #----mean month, season and year net temperature and depth----
    ...
    # year_temp= dplyr:: select(all_data,Year,Temp)
...
#--mean month depth and temperature
    ...
    month_depth=month_depth[month_depth$Year %in% c(1987:2018),]
    
    #mean month net temperature - same as month depth [seperate because the NA rows doesn't equal]
    # month_temp=dplyr:: select(all_data,Year,Month,Temp)
    ...
    # month_temp=na.omit(month_temp)
    # month_temp=dplyr:: select(month_temp,Year=Group.1,Month=Group.2,Temp)
    # 
...
#---mean season depth and temperature
    ...
    # winter_temp=dplyr:: select(winter_temp,Year,Temp,Depth)
    ...
    # spring_temp=dplyr:: select(spring_temp,Year,Temp,Depth)
    ...
    # summer_temp=dplyr:: select(summer_temp,Year,Temp,Depth)
    ...
    # autumn_temp=dplyr:: select(autumn_temp,Year,Temp,Depth)
    ...
    # 
     ...
     #mean depth for any season
    #choose the winter data  
    winter_depth=month_depth[month_depth$Month %in% winter_vec,]
    #mean temperature and depth by year
    winter_depth=aggregate.data.frame(winter_depth,list(winter_depth$Year),FUN = mean,drop = F)
    #remove the calcukations columns
    winter_depth=dplyr:: select(winter_depth,Year,Depth)
    #join column with the season name 
    winter_depth=cbind("Season"=rep("winter"),winter_depth)
    
    spring_depth=month_depth[month_depth$Month %in% spring_vec,]
    spring_depth=aggregate.data.frame(spring_depth,list(spring_depth$Year),FUN=mean,drop = F)
    spring_depth=dplyr:: select(spring_depth,Year,Depth)
    spring_depth=cbind("Season"=rep("spring"),spring_depth)
    
    summer_depth=month_depth[month_depth$Month %in% summer_vec,]
    summer_depth=aggregate.data.frame(summer_depth,list(summer_depth$Year), FUN = mean,drop = F)
    summer_depth=dplyr:: select(summer_depth,Year,Depth)
    summer_depth=cbind("Season"=rep("summer"),summer_depth)
    
    autumn_depth=month_depth[month_depth$Month %in% autumn_vec,]
    autumn_depth=aggregate.data.frame(autumn_depth,list(autumn_depth$Year),FUN=mean,drop = F)
    autumn_depth=dplyr:: select(autumn_depth,Year,Depth)
    autumn_depth=cbind("Season"=rep("autumn"),autumn_depth)
    #join all seasons to 1 data frame
    season_depth=rbind(winter_depth,spring_depth,summer_depth,autumn_depth)
...
#start from 1987
annual_mtc=annual_mtc[annual_mtc$index %in% c(1987:2018),]
year_depth = year_depth[year_depth$Year %in% c(1987:2018),]

#bind mean net temperature and mean depth
#annual_mtc=cbind("mtc"=annual_mtc$mtc,year_temp_depth)


annual_mtc=cbind("mtc"=annual_mtc$mtc,year_depth)

#start from 1991
#annual_mtc=annual_mtc[annual_mtc$Year %in% c(1987:2018),]
...
mediter_annual_mtc=mediter_annual_mtc[mediter_annual_mtc$index %in% c(1987:2018),]
red_annual_mtc=red_annual_mtc[red_annual_mtc$index %in% c(1987:2018),]
...
#start in 1991
#mediter_annual_mtc=mediter_annual_mtc[mediter_annual_mtc$Year %in% c(1991:2018),]
#red_annual_mtc=red_annual_mtc[red_annual_mtc$Year %in% c(1991:2018),]


plot(mediter_annual_mtc, main="Mediterrinian Annual mtc")
plot(red_annual_mtc,main="Red Annual mtc")

# ggplot(mediter_annual_mtc, aes(x=Temp,y=mtc,colour=Year, size=3))+
#   geom_point()+
...
# ggplot(mediter_annual_mtc, aes(x=Year,y=mtc,colour=Temp, size=3))+
#   geom_point()+
#   ggtitle("Annual Mediterranian species MTC by year")
# 
# ggplot(red_annual_mtc, aes(x=Temp,y=mtc,colour=Year, size=3))+
#   geom_point()+
...
# ggplot(red_annual_mtc, aes(x=Year,y=mtc,colour=Temp, size=3))+
#   geom_point()+
...
```{r}
 #call the season catch function
  ...
  #start from 1987
  season_all_mtc=season_all_mtc[season_all_mtc$index %in% c(1987:2018),]
  ...
  season_all_mtc=full_join(season_all_mtc,season_depth,by=c("Year","Season"))
 ...
 #start from 1987
 season_mediter_mtc=season_mediter_mtc[season_mediter_mtc$index %in% c(1987:2018),]
 season_red_mtc=season_red_mtc[season_red_mtc$index %in% c(1987:2018),]
 ...
 #bind mean net temperature and mean depth - NA in the missing values 
 season_mediter_mtc=full_join(season_mediter_mtc,season_depth,by=c("Year","Season"))
 season_red_mtc=full_join(season_red_mtc,season_depth,by=c("Year","Season"))
...
par(mfrow = c(2,2)) 
  ...
  mediter_gam_season = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH) + s(Season_num, bs = "cc", k=4) ,data = net_mediter_mtc)
   # +s(Date_num, bs = "gp")
  
red_gam_season = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH) + s(Season_num, bs = "cc", k=4) ,data = net_red_mtc)
...
par(mfrow = c(2,2)) 
  ...
  #---month model
  mediter_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH) + s(Month, bs = "cc", k=12) ,data = net_mediter_mtc)
  # 
  red_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(loc_DEPTH) + s(Month, bs = "cc", k=12) ,data = net_red_mtc)
#
...
#------ compare net temperature -----

compare the net temperature accepted from Copernicus MOdel to Heker Yamim observasions.


```{r}
#upload diary data
old_data = read.csv("prosseced data/final_data_fishes_known_origin_1987_2013.CSV")
#upload Heker Yamim observations - near Hadera, 26 m depth, daily mesurments
observ = read.delim("data/ADCP_temp_2004_2018.txt")

#rename column names
observ = observ %>% rename(date = X.Date,Temp = Daily.Average.Bottom.Temperaure.C..deg..26.m..depth )


#seperate the date to 3 columns
observ$date = as.character(observ$date)
  observ$date = gsub(" ", ":",observ$date)
    observ$date = gsub(":00:00:00", "",observ$date)
      observ = observ %>% separate(date, c("Year","Month", "Day"), sep = ":") 

    
#uploud and seperate date to 3 columns Hadera 1994-2003 temperature data
old_observ = read.csv("data/Hadera_Water_Temperature_1994_2003_11m_depth.CSV")

old_observ = old_observ %>% rename(date = Date.Time.GMT.
, Temp = Value.of.hourly.average.sea.water.temperature.measured.at.depth.11.6.m..2km.from.shore.line..Hadera....degrees.Celsius) 

old_observ$date = as.character(old_observ$date)
old_observ = old_observ %>% separate(date, c("Date","Time"), sep = " ") %>% dplyr::select(Date:Temp)%>% separate(Date, c("Day","Month", "Year"), sep = "/") 

      
  #clean nonsense measurmets and create mean year and month mean  
   observ = observ %>% dplyr:: filter(temp>0)
      year_observ = observ %>% dplyr::group_by(Year) %>% summarise(YearT = mean(temp)) %>% ungroup()
        month_observ = observ %>% dplyr::group_by(Year,Month) %>% summarise(MonthT = mean(temp)) %>% ungroup()
    
    old_observ = old_observ %>% dplyr:: filter(!is.na(Temp))
      year_old_observ = old_observ %>% dplyr::group_by(Year) %>% summarise(YearT = mean(Temp)) %>% ungroup()
        month_old_observ = old_observ %>% dplyr::group_by(Year,Month) %>% summarise(MonthT = mean(Temp)) %>% ungroup()

    write.csv(year_observ,"prosseced data/year_temp_ADCP.CSV")
      write.csv(month_observ,"prosseced data/month_temp_ADCP.CSV")
    
    write.csv(year_old_observ,"prosseced data/year_temp_Pacer.CSV")
      write.csv(month_old_observ,"prosseced data/month_temp_Pacer.CSV")
  
    year_temp_paper = read.csv("data/year temperature paper.CSV")
    
    year_temp_hadera = rbind(year_old_observ,year_observ)
    plot(year_temp_hadera)
    
#take just the dates included in the diary data
observ$Month = as.numeric(observ$Month)
drop_observ = observ %>% dplyr::filter(Year =="2013") %>%dplyr::filter(Month <=9)
observ = observ %>%filter(Year <=2012) 
observ = rbind(observ,drop_observ)

#take just the date included in Heker data, and depthes from 16-36 meter.
old_data = old_data %>% dplyr::select(Year,Month,Day,depth_meter,Temp) %>% filter(Year>=2004) %>% filter((depth_meter > 15) & (depth_meter<37))

#cerate date column
  observ$Date<-paste(observ$Year,observ$Month,observ$Day,sep = "-")
  old_data$Date<-paste(old_data$Year,old_data$Month,old_data$Day,sep = "-")
  
    #order the column by year month day
    observ$Date<-ymd(observ$Date)
    old_data$Date<-ymd(old_data$Date)
    
    #I didn't finish yet. need to: 
    #1. join togother the nets in the same day in the old_data
    #2. remove remove from observ data row doiesn't exist in the old data
    #3. compare temperaturs.
    
    
  old_data = old_data %>% group_by(Date) %>% summerize(Maen_temp = sum(Temp)) %>%
