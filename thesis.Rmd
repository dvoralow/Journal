...
mediter_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cc"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_mediter_mtc)

#without interaction
mediter_catch_gam = gamm(Sum ~ s(Month, bs = "cc", k=12) + s(Year, bs = "cr")  +s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cc"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_mediter_mtc)
  ...
  plot(  mediter_catch_gam$gam,se=TRUE,col = "blue")
   ...
   red_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cc"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_red_mtc)
   
   #without interaction
    red_catch_gam = gamm(Sum ~ s(Month, bs = "cc", k=12) + s(Year, bs = "cr")  +s(Depth)+ (Fisherman)+ s(Start_hh, bs = "cc"),correlation=corARMA(form = ~Net_num|YearMonth,p=3), data = net_red_mtc)
...
all_catch_gam = gamm(Sum ~ te(Month, bs = "cc", k=12) + te(Year, bs = "cr")  + ti(Month,Year, bs = c("cc","cr"))+ s(Start_hh, bs = "cc")+s(Depth)+ (Fisherman),correlation=corARMA(form = ~Net_num|YearMonth,p=1), data = all_dataF)
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    ...
    labs(title = abondance_species_subnames[i,3], 
         subtitle = abondance_species_subnames[i,4],
         x = NULL,
         y = "Relative catch") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)+
  theme_update(text = element_text(size=20))+
...
#---loop - create seasonal graphs, without interaction, for interactions' species
...
p1 <- ggplot(pdata, aes(x = Month, y = Fitted, colour = Year,group = factor(Year))) +
    ...
    labs(title = int_species_subnames[i,3], 
         subtitle = int_species_subnames[i,4],
         x = NULL,
         y = "Relative catch") +
    scale_x_continuous(breaks = 1:12, minor_breaks = NULL, labels = month.abb)+
  theme_update(text = element_text(size=20))+
...
```{r}
season_species_b = c("DLS", "MUL", "Sud","Sja","SPH","Sd","Umo")
b_size_all = b_size_all %>%dplyr::filter(species %in% c("DLS", "MUL","Sud","Sja","SPH","Sd","Umo"))

season_b_subname = scientific_namesF %>% dplyr::select(Sp_code, Common_name, Scientific_name = scintific_name)
%>% filter(Sp_code %in% season_species_b)
write.csv(season_species_b,"prosseced data/season_b_subname.CSV")

species_b_subname = read.csv("prosseced data/species_b_subname.CSV")
...
write.csv(aic_species_data, "prosseced data/aic_bsize.CSV")
