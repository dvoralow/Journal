...
net_all_data = net_all_data %>% filter(Year > 1990) %>% dplyr::select(-Depth) %>%
   rename(Depth = depth_meter) %>%  dplyr:: filter(!is.na(mtc))  %>% dplyr::filter(Depth<100) %>% filter(Gambari_net==FALSE)
#---seperate by origin---
...
#get out rows without mtc and depth deepre than 100 m 
 ...
 net_red_mtc = net_red_mtc %>% filter(Year > 1990) %>% dplyr::select(-Depth) %>%
   rename(Depth = depth_meter) %>%  dplyr:: filter(!is.na(mtc))  %>% dplyr::filter(Depth<100) %>% filter(Gambari_net==FALSE)
...
net_all_data = net_all_data %>% mutate(Date_num = as.numeric(Date))
gam_all = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + (Fisherman)+ s(Start_hh, bs = "cr")  + 
                          s(Date_num, bs = "gp") +  s(Month, bs = "cc", k=12),data =net_all_data)

gam_all = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + 
...
plot(depth_gam,se=TRUE,col = "blue")
  ...
  #---------month model---
 net_mediter_mtc = net_mediter_mtc %>% mutate(Date_num = as.numeric(Date))

  mediter_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + (Fisherman)+ s(Start_hh, bs = "cr")  + 
                          s(Date_num, bs = "gp") +  s(Month, bs = "cc", k=12),data =net_mediter_mtc)
  ...
  net_red_mtc = net_red_mtc %>% mutate(Date_num = as.numeric(Date))
    net_red_mtc = net_red_mtc %>% filter(Year>1999)

  
  red_gam_month = gam(mtc ~ s(Year, bs = "cr") + s(Depth) + (Fisherman)+ s(Start_hh, bs = "cr")+
                          s(Date_num, bs = "gp")  + s(Month, bs = "cc", k=12) ,data = net_red_mtc)
...
#create Sum columns - sum of the catch of any net
 ...
 net_red_mtc = net_red_mtc %>% dplyr::mutate(Sum = cbind(catch_net_sum)) 
  ...
  #net cath model
  mediter_catch_gam = gam(Sum ~ s(Year, bs = "cr") +s(Month, bs = "cc", k=12)+s(Depth), data = net_medietr_mtc)
