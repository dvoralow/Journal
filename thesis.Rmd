...
ggplot(mediter_annual_mtc, aes(x=Year ,y=mtc,colour=Depth, size=3))+
  geom_point()+#geom_smooth(method=lm,se=FALSE,colour="black",size = 1)+  

    scale_size(guide = 'none')+
  theme(text = element_text(size=20),axis.text.x = element_text(angle=90, hjust=1))+
  labs(y ="MTC")+
 scale_x_continuous (breaks=seq(1991,2018, by = 2))+ 
  scale_colour_gradientn(colours=topo.colors(7))+
  geom_line(aes(y = SST_anom+20.5, colour = SST_anom,size = 1))+
  scale_y_continuous(sec.axis = sec_axis(~.-20.5, name = "SST Anomaly"))+
  ggtitle("Annual Mediterranian Species MTC By Depth, Compare To SST Anomaly")+
  stat_smooth(aes(y = mtc), 
              method = "lm", formula = formula, se = FALSE, size = 1.5, color = "green",linetype = "twodash") +
  stat_smooth(aes(y =  SST_anom+20.5), 
              method = "lm", formula = formula, se = FALSE, size = 1, color = "blue",linetype = "twodash")+
geom_text(data =mediter_annual_mtc[mediter_annual_mtc$Year =="2004", ], aes(label = "MTC"), hjust = 1,vjust = 1.5)+
